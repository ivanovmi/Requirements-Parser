...
parameters = {'openid.assoc_handle': '',
              ...
              'openid.ext2.type.Email':
                  'http://schema.openid.net/contact/email',
              'openid.ext2.type.FirstName':
                  'http://schema.openid.net/namePerson/first',
              'openid.ext2.type.LastName':
              ...
              'openid.return_to':
                  'https://review.fuel-infra.org/OpenID?gerrit.mode=SIGN_IN&'
...
def login_to_launchpad(launchpad_id, launchpad_pw):
    ...
    login_data = {'openid.usernamesecret': '',
                  'csrfmiddlewaretoken': requests.utils.dict_from_cookiejar(
                      s.cookies)['csrftoken'],
                  'email': launchpad_id,
    ...
    Cookies = 'C=1; csrftoken=' + \
              requests.utils.dict_from_cookiejar(s.cookies)['csrftoken']
    ...
    cookies = 'csrftoken=' + \
              requests.utils.dict_from_cookiejar(s.cookies)['csrftoken']
    try:
        cookies += '; C=1; sessionid=' + \
                   requests.utils.dict_from_cookiejar(s.cookies)['sessionid']
    except KeyError:
        print 'Could not authenticate'
        raise SystemExit
    cookies += '; openid_referer=' \
               '"https://review.fuel-infra.org/login/q/status:open"'

    ss.headers.update({'Referer':
                       'https://review.fuel-infra.org/login/q/status:open'})
    ss.headers.update({'Cookie': cookies})
    r = ss.post('https://login.launchpad.net/+openid',
                data=parameters, allow_redirects=True)

    d_data = {'csrfmiddlewaretoken': requests.utils.dict_from_cookiejar(
        s.cookies)['csrftoken'],
        'email': 'on',
        'fullname': 'on',
        'ok': '',
    ...
    # header = s.headers
    # rd =
    ss.post(r.url, data=d_data, allow_redirects=True)

    if 'GerritAccount' in requests.utils.dict_from_cookiejar(ss.cookies):
